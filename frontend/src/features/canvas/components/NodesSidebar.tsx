      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-4 custom-scrollbar">
        {totalCount === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 gap-2 text-center">
            <p className="text-sm text-[var(--sea-ink-soft)]">No nodes yet.</p>
            <p className="text-xs text-[var(--sea-ink-soft)] opacity-70">Add nodes from the toolbar above.</p>
          </div>
        ) : (
          sections.map((section) => (
            <div key={section.label}>
              ...
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-[var(--sea-ink-soft)]">
                  {section.label}
                </span>
              ...
              {section.nodes.length === 0 ? (
                <p className="text-xs text-[var(--sea-ink-soft)] px-2 py-1 opacity-60">None added yet</p>
              ) : (
                    ...
                    return (
                      <li key={node.id}>
                        <button
                          ...
                          className={`w-full text-left flex items-center gap-2 px-3 py-2.5 rounded-xl transition-all duration-150 group cursor-pointer ${
                            isSelected
                          ...
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-sm font-medium truncate ${
                                isSelected ? 'text-violet-700 dark:text-violet-300' : 'text-[var(--sea-ink)]'
                              }`}
                            >
                            ...
                            {sub && (
                              <p className="text-xs text-[var(--sea-ink-soft)] truncate flex items-center gap-1 mt-0.5">
                          ...
                          <ChevronRight
                            className={`w-3.5 h-3.5 shrink-0 transition-transform ${
                              isSelected
                                ...
                                : 'text-[var(--sea-ink-soft)] opacity-0 group-hover:opacity-100'
                            }`}
                          />
                        </button>
                      </li>
      ...
      <Panel position="bottom-left" className="md:hidden mb-14">
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <button
              className="island-shell flex items-center gap-2 px-3 py-2.5 rounded-full text-sm font-medium text-[var(--sea-ink)] shadow-lg transition-colors cursor-pointer"
          ...
          <SheetContent side="left" className="w-72 p-0 border-[var(--line)] bg-[var(--surface)] flex flex-col">
            <SheetHeader className="px-4 pt-5 pb-3 border-b border-[var(--line)]">
              <SheetTitle className="text-base text-[var(--sea-ink)]">Canvas Nodes</SheetTitle>
            </SheetHeader>
  ...
  return (
    ...
    >
      <div className="island-shell h-full flex flex-col rounded-none rounded-r-2xl border-l-0 overflow-hidden shadow-xl">
        {/* Header */}
        <div className="flex items-center gap-2 px-3 py-4 border-b border-[var(--line)] shrink-0">
          {!collapsed && (
            <span className="flex-1 text-sm font-semibold text-[var(--sea-ink)] truncate">
              Nodes <span className="text-[var(--sea-ink-soft)] font-normal">({totalCount})</span>
            </span>
          )}
          <button
        ...
        {collapsed && (
          <div className="flex flex-col items-center gap-3 pt-4">
            <BookOpen className="w-4 h-4 text-violet-500" title="Story Beats" />
            <User className="w-4 h-4 text-fuchsia-500" title="Characters" />
            <Shield className="w-4 h-4 text-indigo-500" title="World Rules" />
          </div>
        )}
      </div>
    </div>
