...
/**
 * A ReactFlow custom node representing a plot event.
 */
export const EventNode = memo(({ id, data, selected }: NodeProps<StoryNodeData>) => {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !border-2 !border-rose-500 !bg-[var(--surface)]" />
      ...
      <div className="flex items-center gap-2 px-4 pt-4 pb-2 border-b border-[var(--line)]">
        ...
        {editing ? (
          <input
            ...
            className="flex-1 text-sm font-semibold bg-transparent outline-none border-b border-rose-400 text-[var(--sea-ink)] min-w-0"
          />
        ) : (
          <span className="flex-1 text-sm font-semibold text-[var(--sea-ink)] truncate">{data.title}</span>
        )}
        <span className="text-[10px] uppercase tracking-widest font-bold text-rose-500 ml-auto flex-shrink-0">Event</span>
      </div>
      ...
      <div className="flex items-center justify-end gap-1 px-3 pb-3">
        {editing ? (
          <>
            <button onClick={saveEdit} className="p-1.5 rounded-lg bg-rose-100 dark:bg-rose-900/40 text-rose-600 hover:bg-rose-200 transition-colors cursor-pointer">
      ...
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !border-2 !border-rose-500 !bg-[var(--surface)]" />
    </div>
  );
});
