...
import { Film, Pencil, Trash2, X, Check, Tag } from 'lucide-react';
import type { NodeProps } from '@xyflow/react';
import type { StoryNodeData } from '#/features/canvas/types/canvas.types';
import { useCanvasStore } from '#/features/canvas/store/useCanvasStore';

/**
 * A ReactFlow custom node representing a story scene.
 * Shows title, content, and tags with inline editing support.
 */
export const SceneNode = memo(({ id, data, selected }: NodeProps<StoryNodeData>) => {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !border-2 !border-violet-500 !bg-[var(--surface)]" />
      ...
      <div className="flex items-center gap-2 px-4 pt-4 pb-2 border-b border-[var(--line)]">
        ...
        {editing ? (
          <input
            ...
            className="flex-1 text-sm font-semibold bg-transparent outline-none border-b border-violet-400 text-[var(--sea-ink)] min-w-0"
          />
        ) : (
          <span className="flex-1 text-sm font-semibold text-[var(--sea-ink)] truncate">{data.title}</span>
        )}
        <span className="text-[10px] uppercase tracking-widest font-bold text-violet-500 ml-auto flex-shrink-0">Scene</span>
      </div>
      ...
      <div className="px-4 py-3">
        ...
        {data.tags && data.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {data.tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300">
      ...
      <div className="flex items-center justify-end gap-1 px-3 pb-3">
        {editing ? (
          <>
            <button onClick={saveEdit} className="p-1.5 rounded-lg bg-violet-100 dark:bg-violet-900/40 text-violet-600 hover:bg-violet-200 transition-colors cursor-pointer">
      ...
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !border-2 !border-violet-500 !bg-[var(--surface)]" />
    </div>
  );
});
